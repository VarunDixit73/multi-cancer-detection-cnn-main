...
import os
import app.skin_predictor as sc
import app.brain_predictor as bt
...
def uploadImagesc():
    if request.method == 'POST':
        ...
        if file and allowed_files(file.filename):
            ...
            result = sc.predict(f"app/static/uploads/{filename}")
...
def uploadImagebt():
    if request.method == 'POST':
        ...
        if file and allowed_files(file.filename):
            ...
            db.session.commit()
            flash('file uploaded and saved','success')
            result = bt.predict(f"app/static/uploads/{filename}")
            session['uploaded_file'] = f"/static/uploads/{filename}"
            print(result)
            session['result'] = result
            return redirect(request.url)
